...
class Vocabulary:
    ...
    def __init__(self) -> None:
        ...
        logit_example: NDArray[np.float32] = np.asarray(
        ...
        self.M_fun_name: NDArray[np.float32] \
            = self._build_fun_name_mask(functions)
    ...
    def _build_chars_mask(self) -> NDArray[np.float32]:
        ...
        Returns:
            ...
            _NEG_INF = blocked.
        """
        mask: NDArray[np.float32] = np.full(
...
class ConstrainedDecoder:
    ...
    def select_function_name(
        ...
        while True:
            ...
            masked_logits: NDArray[np.float32] = logits + vocab.M_fun_name
    ...
    def select_arguments(
        ...
        for name, param in function.parameters.items():
            ...
            if param_type == "integer" and isinstance(arg, (int, float)):
                arg = round(arg)
    ...
    def generate_number(self, prompt: str) -> float:
        ...
        for _ in range(_MAX_TOKENS):
            ...
            mask: NDArray[np.float32] = self._build_number_mask(
            ...
            if any(c in self._clean_token(next_str) for c
                   in {" ", "\n", ",", "}", "]", "\t"}):
                break
    ...
    def generate_str(
        ...
        for _ in range(_STRING_MAX):
            ...
            logits: NDArray[np.float32] \
                = np.asarray(raw_logits, dtype=np.float32)
            masked_logits: NDArray[np.float32] = logits + vocab.M_chars
    ...
    def _generate_boolean(self, prompt: str) -> bool:
        ...
        logits: NDArray[np.float32] = np.asarray(raw_logits, dtype=np.float32)
        mask: NDArray[np.float32] = self._build_boolean_mask(len(logits))
    ...
    def _build_number_mask(
        ...
        """
        mask: NDArray[np.float32] = np.zeros(logits_len, dtype=bool)
    ...
    def _build_boolean_mask(self, logit_len: int) -> NDArray[np.float32]:
        ...
        """
        mask: NDArray[np.float32] = np.zeros(logit_len, dtype=bool)
        valid: Set[str] = {
    ...
    @staticmethod
    def _apply_mask(
        logits: NDArray[np.float32], mask: NDArray[np.float32]
    ) -> NDArray[np.float32]:
    ...
    @staticmethod
    def _clean_token(token_str: str) -> str:
        """Norma lize BPE space markers in a token string.
